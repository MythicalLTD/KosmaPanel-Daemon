...
def sqlquery(sql, *parameter):
    conn = sqlite3.connect("/etc/KosmaPanel/old/database.sqlite", check_same_thread=False)
    cursor = conn.cursor()
...
def route_create_server(uuid):
    ...
    if system_token == app.config["SYSTEM_TOKEN"]:
        try:
            os.mkdir("/etc/KosmaPanel/old/data/{}".format(uuid))
...
try:
    if sys.argv[1] == "--token":
        ...
        conn.commit()
        if not os.path.exists("/etc/KosmaPanel/old/data"):
            os.mkdir("/etc/KosmaPanel/old/data")
    ...
    if os.path.isfile("database.sqlite"):
        ...
        app.config["UPLOAD_FOLDER"] = "/etc/KosmaPanel/old/data"
